...
if __name__ == "__main__":
    ...
    def plot_joint_linear(fig, ax, z_raw, title, xlabel, ylabel, cmap):
        ...
        z  = _roll2d(z_raw)
        im = ax.pcolormesh(phi_plot, phi_plot, z,
    ...
    def plot_joint_ln(fig, ax, z_raw, title, xlabel, ylabel, cmap):
        ...
        im = ax.pcolormesh(phi_plot, phi_plot, ln_z,
    ...
    for normalised in [True, False]:
        ...
        for n in ALL_NMERS:
            ...
            for col, (j_arr, title, cmap) in enumerate([
                (jf_n, f"P(φ{N},φ1) — Reverse (tail→head)",  cmap_fwdrev),
                (jr_n, f"P(φ{N},φ1) — Forward (head→tail)",  cmap_fwdrev),
    ...
    for normalised in [True, False]:
        ...
        for n in ALL_NMERS:
            ...
            # Joints — ln scale
            for col, (j_arr, title, cmap) in enumerate([
                (jf_n, f"ln P(φ{N},φ1) — Reverse (tail→head)",  cmap_fwdrev),
                (jr_n, f"ln P(φ{N},φ1) — Forward (head→tail)",  cmap_fwdrev),
